...
let clicked = await page.evaluate(() => {
  const row = [...document.querySelectorAll('.slashrow')].find(
    (r) => r.querySelector('.slashcmd')?.textContent === '/status',
  )
  if (!row) return false
  row.click()
  return true
...
if (!clicked) {
  await page.type('.slashfilter', 'status')
  await sleep(400)
...
for (let i = 0; i < 40; i++) {
  ...
  const st = await page.evaluate(() => ({
    ...
    lines: [...document.querySelectorAll('.cfgrow')].map((e) => e.innerText.replace(/\n/g,' = ')),
...
for (let i = 0; i < 40 && still < 6; i++) {
  await sleep(1000)
  const n = await page.evaluate(() => document.querySelectorAll('.cfgrow').length)
